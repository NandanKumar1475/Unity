...
namespace Whisper.Utils
{
    ...
    public class MicrophoneRecord : MonoBehaviour
    {
        ...
        // Default for lower latency demo: smaller chunk size -> more frequent sends
        [Tooltip("Length of audio chunks in seconds, useful for streaming")]
        public float chunksLengthSec = 0.25f; // changed from 0.5 -> 0.25 for lower latency
        ...
        // check more often for faster reaction
        [Tooltip("How often VAD checks if current audio chunk has speech")]
        public float vadUpdateRateSec = 0.06f; // changed from 0.1 -> 0.06

        // smaller VAD context for lower latency demos
        ...
        public float vadLastSec = 0.8f; // reduced from 1.25 -> 0.8
        ...
        // shorter stop time for demo — tune to reduce waiting on silence
        [Tooltip("After how many seconds of silence microphone will stop record")]
        public float vadStopTime = 1.2f; // changed from 3 -> 1.2 (demo-friendly)
        ...
        public int ClipSamples => _clip != null ? _clip.samples * _clip.channels : 0;
        ...
        private void Awake()
        {
            ...
            {
                var options = AvailableMicDevices
                ...
                int defaultIndex = microphoneDropdown.options.FindIndex(op => op.text == microphoneDefaultLabel);
                microphoneDropdown.value = defaultIndex >= 0 ? defaultIndex : 0;
        ...
        private void Update()
        {
            ...
            if (micPos < 0)
            {
                // mic not ready or error; bail
                LogUtils.Verbose($"Microphone.GetPosition returned {micPos}");
                return;
            }
        ...
        private void UpdateVad(int micPos)
        {
            ...
            // set _lastVadPos using micPos (not samplesCount)
            _lastVadPos = micPos;
        ...
        public void StartRecord()
        {
            ...
            // Try/catch in case Microphone.Start fails on some platforms
            try
            {
            ...
            {
                LogUtils.Error($"Microphone.Start failed: {ex}");
                _clip = null;
            }

            if (_clip == null)
            {
                LogUtils.Error("Microphone clip is null after Microphone.Start. Recording aborted.");
                IsRecording = false;
                return;
            }
            ...
            // compute chunk size in samples, ensure at least 1 sample
            _chunksLength = (int) (_clip.frequency * _clip.channels * Mathf.Max(0.001f, chunksLengthSec));
            if (_chunksLength < 1) _chunksLength = 1;
        }
        ...
        public void StopRecord(float dropTimeSec = 0f)
        {
            ...
            var finalAudio = new AudioChunk()
            {
                ...
                Frequency = _clip != null ? _clip.frequency : frequency,
                IsVoiceDetected = IsVoiceDetected,
                Length = (float) data.Length / ((_clip != null ? _clip.frequency : frequency) * (_clip != null ? _clip.channels : 1))
            };
            ...
            // capture channel/frequency safely before destroying clip
            int capturedChannels = _clip != null ? _clip.channels : 1;
            int capturedFrequency = _clip != null ? _clip.frequency : frequency;
            ...
            // play echo sound (use captured values)
            if (echo)
            {
                try
                {
                    var echoClip = AudioClip.Create("echo", data.Length,
                        capturedChannels, capturedFrequency, false);
                    echoClip.SetData(data, 0);
                    PlayAudioAndDestroy.Play(echoClip, Vector3.zero);
                }
                catch (Exception ex)
                {
                    LogUtils.Error($"Failed to create/play echo clip: {ex}");
                }
            }
